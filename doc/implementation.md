...
## Phase 12: PWA実装（予定工数: 3時間）

### タスク

#### 【 】12-1. manifest.json作成（30分）
- **Red**: manifest.json読み込みテスト
- **Green**: manifest.json実装
  - アプリ名、説明、スタートURL
  - テーマカラー、背景色
  - アイコン定義（192x192、512x512）
- **Refactor**: メタデータ最適化

#### 【 】12-2. Service Worker実装（1.5時間）
- **Red**: Service Worker登録テスト
- **Green**: `public/sw.js` 実装
  - キャッシュファースト戦略（静的アセット）
  - ネットワークファースト戦略（API呼び出し）
  - オフライン対応
- **Refactor**: キャッシュ戦略最適化

#### 【 】12-3. アイコン配置・設定（30分）
- ユーザー提供アイコンを`public/`に配置
  - `icon-192x192.png`
  - `icon-512x512.png`
- manifest.json とリンク
- **動作確認**: Chrome DevTools でPWA診断

#### 【 】12-4. PWA動作確認（30分）
- **Red**: PWAインストールテスト
- **Green**: ホーム画面追加動作確認
- **Refactor**: スプラッシュスクリーン調整
...
### PWA品質
- [ ] Lighthouseスコア: PWA 100点
- [ ] ホーム画面に追加可能
- [ ] オフライン動作（LocalStorageデータ閲覧可能）
- [ ] アプリライクな起動体験
